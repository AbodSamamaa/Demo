	...
	public class TaxService : ITaxService
	{
		...
		public async Task<Tax> Add(Tax entity)
		{
			...
			try
			{
				...
				if (Tax_created.IdTaxes == 0)
					throw new TaskCanceledException("Failed to create Tax");

				IQueryable<Tax> query = await _repository.Query(u => u.IdTaxes == Tax_created.IdTaxes);
		...
		public async Task<Tax> Edit(Tax entity)
		{
			...
			Tax Tax_exists = await _repository.Get(u => u.Percentage == entity.Percentage && u.IdTaxes != entity.IdTaxes);
			...
			try
			{
				IQueryable<Tax> queryTax = await _repository.Query(u => u.IdTaxes == entity.IdTaxes);
		...
		public async Task<bool> Delete(int idTax)
		{
			try
			{
				Tax Tax_found = await _repository.Get(u => u.IdTaxes == idTax);
